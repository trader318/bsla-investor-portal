...
const categories: Category[] = [
  ...
  },
  {
    ...
    documents: [
      { name: 'BSLA — Energy Ready Sites', meta: 'PDF · Pipeline Overview', type: 'pdf', viewHref: '#', downloadHref: '#' },
    ],
  },
  {
    key: 'corporate',
  ...
  },
  {
    ...
    documents: [{ name: 'Management & Leadership Team', meta: 'DOCX · Team Bios', type: 'docx', downloadHref: '#' }],
  },
  {
    key: 'market',
    num: '08',
...
function DocumentCard({ doc }: { doc: DocumentItem }) {
  return (
    <div className="dr-doc-card">
      <div className={`dr-doc-icon ${doc.type}`}>
        <span className="dr-doc-icon-label">{doc.type.toUpperCase()}</span>
      </div>
      ...
      <div className="dr-doc-actions">
        {doc.viewHref && (
          <a href={doc.viewHref} className="dr-doc-btn dr-doc-btn-view">
            View
          </a>
        )}
        <a href={doc.downloadHref || '#'} className="dr-doc-btn dr-doc-btn-dl">
          Download
        </a>
      </div>
    </div>
      ...
      <div className="dr-layout">
        <div className="dr-topbar">
          <div className="dr-topbar-left">
            <div className="dr-topbar-wm">
              <span className="dr-topbar-wm-top">BIGSTAR</span>
              <span className="dr-topbar-wm-bot">LAND ACQUISITION</span>
            </div>
            <div className="dr-topbar-divider" />
            <span className="dr-topbar-label">Private Deal Room</span>
          </div>
          <div className="dr-topbar-right">
            <span className="dr-topbar-user">
              Welcome, <strong>Investor</strong>
            </span>
            <span className="dr-topbar-badge">Verified · Accredited</span>
          </div>
        </div>
        ...
        <aside className="dr-sidebar">
          <div className="dr-sidebar-section">
            ...
            <nav className="dr-sidebar-nav">
              <button className={`dr-nav-item ${activeCategory === 'all' ? 'active' : ''}`} onClick={() => setActiveCategory('all')}>
                <span className="dr-nav-num" />
                <span className="dr-nav-text">All Documents</span>
                <span className="dr-nav-count">{totalDocs}</span>
              </button>
              {categories.map((category) => (
                <button
                  key={category.key}
                  className={`dr-nav-item ${activeCategory === category.key ? 'active' : ''}`}
                  onClick={() => setActiveCategory(category.key)}
                >
                  <span className="dr-nav-num">{category.num}</span>
                  <span className="dr-nav-text">{category.navTitle || category.title}</span>
                  <span className="dr-nav-count">{category.documents.length}</span>
                </button>
              ))}
            </nav>
          </div>
          ...
          <div className="dr-contact-card">
            ...
            <div className="dr-contact-body">
              ...
              <a href="#" className="dr-contact-book">
                Book a Call
              </a>
            </div>
          </div>
        ...
        <main className="dr-main">
          {activeCategory === 'all' && (
            <div className="dr-welcome">
              ...
              <div className="dr-welcome-content">
                ...
                <p>
                  All materials in this room are confidential and proprietary. Do not share, distribute, or disclose any
                  documents without written consent from BSLA, LLC.
                </p>
                <div className="dr-welcome-stats">
                  <div>
                    <div className="dr-welcome-stat-val">$7.5M</div>
                    <div className="dr-welcome-stat-label">Raise Target</div>
                  </div>
                  <div>
                    <div className="dr-welcome-stat-val">{totalDocs}</div>
                    <div className="dr-welcome-stat-label">Documents</div>
                  </div>
                  <div>
                    <div className="dr-welcome-stat-val">{categories.length}</div>
                    <div className="dr-welcome-stat-label">Categories</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          {activeCategory === 'all' ? (
            <div className="dr-all-cats">
              {categories.map((category) => (
                <section className="dr-cat-block" key={category.key} id={`cat-${category.key}`}>
                  <div className="dr-cat-block-header">
                    ...
                    <span className="dr-cat-block-count">
